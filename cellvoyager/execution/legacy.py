        ...
        response = self.client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    ...
                    "(1) Most workflow steps have ordering dependencies — reduce_dimension(cds) requires preprocess_cds(cds); cluster_cells(cds) requires reduce_dimension(cds); learn_graph(cds) requires cluster_cells(cds); order_cells(cds) requires learn_graph(cds) and a root (root_pr_nodes/root_cells). plot_cells(cds, color_cells_by='pseudotime') needs order_cells first; color_cells_by='cluster' needs cluster_cells first. BUT the provided cds is already preprocessed/aligned/reduced/clustered (and may carry cell-type annotation columns in colData — check colnames(colData(cds))), so those prerequisites are already satisfied — NEVER re-run preprocess_cds/align_cds/reduce_dimension/cluster_cells (not on cds and not on a subset): it overwrites the curated embedding, renumbers clusters, invalidates any existing cell-type annotation columns, and is slow (a common cause of timeouts). Use the existing reducedDims(cds)/clusters(cds)/partitions(cds) and any annotation columns; a subset cds[, cells] keeps them. If a fix is failing because a prerequisite seems missing, check reducedDimNames(cds)/clusters(cds) first — do not recompute. "
                    ...
                    "(4) Differential expression: prefer top_markers(cds, group_cells_by=<colData column>, cores=1) for marker/DE work — it is robust and needs no model. NEVER subset cells before top_markers(): run it on the full cds and choose groups via group_cells_by (it compares each group against the rest of the dataset, so subsetting first yields meaningless/empty markers). Use fit_models() ONLY for an explicit condition contrast, and make it defensive: subset genes with filter_cds_genes(cds[, cells], num_cells_expressed_cutoff=10); ensure the grouping variable has >=2 levels present (colData(cds_sub)$grp <- droplevels(factor(...)); stopifnot(nlevels(...) >= 2)); after coefficient_table(), inspect unique(ct$term) and colnames(ct) and NEVER guess them; filter to status=='OK' and the actual non-intercept term; select columns via intersect(c('gene_short_name','term','estimate','p_value','q_value'), colnames(ct)). (Do not reprocess for DE — see rule 1.) "
                    "(5) Inspect metadata before subsetting/grouping — never guess colData column names or category values. If a cell subset is empty (ncol(cds_sub)==0) or a column/level is not found, the FILTER is wrong: print colnames(colData(cds)) and table()/unique() of the candidate column, use the EXACT name/level (e.g. a cell line may be in 'cellline_detail', not 'Cell.Line'; cluster labels come from clusters(cds), and a cell-type annotation column may or may not exist in colData(cds) — check colnames(colData(cds)), do not assume 'assigned_cell_type'). Do not invent column or category names. Do NOT retry the same filter or guess another column name. Prefer operating on the full cds with grouping over carving out subsets. "
                    "(6) Keep the fix as R/monocle3 — do NOT convert it to Python/scanpy. Provide only fixed code with no explanation."
                )},
                {"role": "user", "content": prompt},
            ],
        )
